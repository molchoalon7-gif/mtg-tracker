...
export default function LoginPage() {
  ...
  async function submit(e:FormEvent){e.preventDefault();setBusy(true);setMessage(""); const db=supabase();
    if(mode==="signup"){
      ...
      const {data,error}=await db.auth.signUp({email,password,options:{data:{display_name:name,username:cleanUsername,phone:cleanPhone},emailRedirectTo:`${window.location.origin}/login`}});
      ...
      setMessage("Account created. Check your email to confirm it, then sign in. Your phone number is stored for matchup coordination but is not SMS-verified.");
    }else{
