...
export default function CreateTournament(){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault(); if(!user)return location.href="/login"; setBusy(true);setMessage(""); const form=new FormData(e.currentTarget);
   ...
   const matches=Number(form.get("matches"));
   const payload={created_by:user.id,name:String(form.get("name")),description:String(form.get("description")||"")||null,format,mode:String(form.get("mode")),starts_at:start.toISOString(),ends_at:end.toISOString(),registration_deadline:deadline.toISOString(),matches_per_player:matches,max_players:form.get("max")?Number(form.get("max")):null};
 ...
 return <><div className="page-head"><div><p className="eyebrow">New tournament</p><h1>Create an event</h1></div><p className="muted">Registration, private decklists, pairings, results and standings are handled for you.</p></div><form className="form panel" onSubmit={submit}>
   ...
   <div className="form-row"><label>Tournament ends<input name="end" type="datetime-local" required/></label><label>Target matches per player<input name="matches" type="number" min="1" max="20" defaultValue="5" required/><span className="hint">Set this now. When registration closes, ManaPair uses the actual player count and builds the fairest unique-opponent schedule at or below this number.</span></label></div>
