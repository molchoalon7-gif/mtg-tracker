alter table public.user_deck_cards
  alter column scryfall_id type uuid using nullif(scryfall_id,'')::uuid,
  alter column oracle_id type uuid using nullif(oracle_id,'')::uuid;
...
begin
  ...
  select
    v_deck_id,
    trim(card->>'card_name'),
    nullif(trim(card->>'scryfall_id'),'')::uuid,
    nullif(trim(card->>'oracle_id'),'')::uuid,
