...
export function FormatPicker({ value, onChange }: { value: string; onChange: (format: string) => void }) {
  ...
  return <>
    <button className="picker-button" type="button" onClick={() => setOpen(true)}><span>{value}</span><span aria-hidden="true">›</span></button>
    {open ? <div className="sheet-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setOpen(false)}>
      <section className="sheet" role="dialog" aria-modal="true" aria-label="Choose tournament format">
        <div className="sheet-head"><div><p className="eyebrow">Tournament format</p><h2>Choose a format</h2></div><button className="sheet-close" type="button" onClick={() => setOpen(false)}>×</button></div>
